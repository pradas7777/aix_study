...
1. 핵심 게시판 5개 (수업 요약(summary), 질문 답변(qna), 정보공유 게시판(lounge),그룹스터디(),복습게시판(preview),그룹스터디(studies))
...
community_db라는 db 만드시면 됩니다

4. DB 연결
방금 만드신 mysql 아이디랑 비번으로 프로젝트 폴더 안에 있는 database.py 파일에서 아랫부분을

(수정전)SQLALCHEMY_DATABASE_URL = "mysql+pymysql://aix:1234tlsfla@localhost:3306/community_db?charset=utf8mb4"

SQLALCHEMY_DATABASE_URL = "mysql+pymysql://본인의mysql아이디:비밀번호@localhost:3306/community_db?charset=utf8mb4" 로 수정하시면 됩니다
